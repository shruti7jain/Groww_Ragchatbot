...
The UI is served directly by the **FastAPI** backend, which exposes a `/api/chat` endpoint connected to the `ResponseBuilder` from Phase 7, and serves the static HTML file at the root route `/`. The entire application is deployed on **Railway**.
